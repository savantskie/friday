...
def send_to_friday(user_input):
    ...
    try:
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
            # ToDo s.settimeout(5)  # fail faster if Friday is unresponsive
            s.connect((HOST, PORT))
            ...
            try:
                ...
                return "[Error: Invalid response from Friday (decode error)]"

    except ConnectionRefusedError:
        return "[Error: Friday is offline or not running]"
    except socket.timeout:
        return "[Error: Friday took too long to respond]"
    except socket.error as e:
        return f"[Error: Communication error with Friday: {str(e)}]"
    except Exception as e:
        return f"[Error: Unexpected error contacting Friday: {str(e)}]"
