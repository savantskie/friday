...
def __init__(self, memory_data_dir: str = "/media/nate/Friday/Friday/memory_data", request=None):
    ...
    def _archive_progress(self):
        """Archive progress file to archives/progress/ with dated name instead of deleting."""
        ...
        try:
            ...
            dst = os.path.join(archive_dir, f"progress_{date_str}.json")
            os.rename(self.progress_file, dst)
    ...
    async def _estimate_batch_size(self, sample_memories: List[Dict],
        ...
        overhead_tokens = len(identity_block.split()) + 50
        ...
        memory_budget = available - identity_tokens - 100
    ...
    async def run_generation(self, user_id: str, model_id: str,
        ...
        resolved_model_id = model_id.strip().lower()

        # Check if a re-scan was flagged — if so, clear archive tracking
        self._check_and_handle_rescan(resolved_user_id, resolved_model_id)
        ...
        await self._write_file_backup_async(accumulated_identity, resolved_user_id)
