    ...
    async def _get_client_tools(self) -> List[Tool]:
        ...
        try:
            ...
            Tool(
                ...
                description="List all available memory banks with memory counts",
                inputSchema={
                    "type": "object",
                    "properties": {}
                }
            ...
            Tool(
                ...
                inputSchema={
                    ...
                    "properties": {
                        "memory_bank": {
                            "type": "string",
                            "description": "Filter tags by specific memory bank (optional)"
                        },
                        "user_id": {
                            "type": "string",
                            "description": "User ID for user separation (required)"
                        },
                        "model_id": {
                            "type": "string",
                            "description": "Model ID for model separation (required)"
                        }
                    },
            ...
            Tool(
                ...
                description="General web search using Brave Search API",
                inputSchema={
                    "type": "object",
                    "properties": {
                        "query": {"type": "string", "description": "Search query"},
            ...
            Tool(
                ...
                inputSchema={
                    ...
                    "properties": {
                        ...
                        "location": {"type": "string", "description": "Location to search around (e.g., 'New York, NY')"},
                        "count": {"type": "integer", "description": "Number of results (1-20)", "default": 10},
    ...
    async def _execute_tool(self, tool_name: str, arguments: Dict[str, Any]) -> CallToolResult:
        ...
        try:
            ...
            if tool_name == "search_memories":
                ...
                result = await self.memory_system.list_available_memory_banks()
            elif tool_name == "list_available_tags":
                try:
                    from tag_manager import TagManager
                    tag_registry_path = os.path.join(os.environ.get("AI_MEMORY_DATA_DIR", "memory_data"), "tag_registry.json")
                    if os.path.exists(tag_registry_path):
                        tm = TagManager()
                        tm.load_registry(tag_registry_path)
                        result = {
                            "status": "success",
                            "tags": dict(tm.registry),
                            "summary": tm.get_registry_summary(),
                            "timestamp": datetime.now(timezone.utc).isoformat()
                        }
                    else:
                        result = {"status": "success", "tags": {}, "summary": {"total_tags": 0, "total_variations": 0, "most_used": []}}
                except Exception as e:
                ...
                result = await self.memory_system.get_error_summary()
            ...
            # SillyTavern-specific tools
                ...
                result = await self.memory_system.trigger_database_maintenance(**arguments)
            elif tool_name == "export_all_tool_calls":
                result = await self.memory_system.export_all_tool_calls(**arguments)
            elif tool_name == "brave_web_search":
                result = await self.memory_system.brave_web_search(**arguments)
            elif tool_name == "brave_local_search":
                result = await self.memory_system.brave_local_search(**arguments)
            elif tool_name == "get_tool_information":
                result = await self.memory_system.get_tool_information(**arguments)
            else:
