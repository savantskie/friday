...
class ConversationDatabase(DatabaseManager):
    ...
    def initialize_tables(self):
        ...
        with self.get_connection() as conn:
            ...
            # Message-Memory Links table - track which messages contributed to which memories
            conn.execute("""
            ...
            # Conversation Processing Queue - updated version with isolation support
            conn.execute("""
                CREATE TABLE IF NOT EXISTS conversation_processing_queue (
                    queue_id TEXT PRIMARY KEY,
                    conversation_id TEXT NOT NULL,
                    processing_type TEXT DEFAULT 'memory_extraction',
            ...
            # Conversation Isolation table - track user/model/character context per conversation
            conn.execute("""
                CREATE TABLE IF NOT EXISTS conversation_isolation (
                    ...
                    created_at TEXT DEFAULT CURRENT_TIMESTAMP,
                    FOREIGN KEY (conversation_id) REFERENCES conversations (conversation_id),
                    UNIQUE(conversation_id, user_id, model_id, character_name)
                )
            """)
    ...
    async def link_message_to_memory(self, message_id: str, memory_id: str) -> str:
        """Link a message to a memory it contributed to"""
        link_id = str(uuid.uuid4())
        
        await self.execute_update(
            """INSERT OR IGNORE INTO message_memory_links (link_id, message_id, memory_id, created_at)
               VALUES (?, ?, ?, ?)""",
            (link_id, message_id, memory_id, get_current_timestamp())
        )
        return link_id
    
    async def get_message_memory_links(self, message_id: str = None, memory_id: str = None) -> List[Dict]:
        """Get links between messages and memories"""
        if message_id:
            rows = await self.execute_query(
                "SELECT * FROM message_memory_links WHERE message_id = ?",
                (message_id,)
            )
        elif memory_id:
            rows = await self.execute_query(
                "SELECT * FROM message_memory_links WHERE memory_id = ?",
                (memory_id,)
            )
        else:
            rows = await self.execute_query("SELECT * FROM message_memory_links")
        
        return [dict(row) for row in rows]
    
    async def add_conversation_to_processing(self, conversation_id: str, processing_type: str = "memory_extraction",
                                            priority: int = 5, user_id: str = None, model_id: str = None,
                                            character_name: str = None) -> str:
        """Queue conversation for processing with isolation context"""
        queue_id = str(uuid.uuid4())
        
        await self.execute_update(
            """INSERT INTO conversation_processing_queue 
               (queue_id, conversation_id, processing_type, priority, user_id, model_id, character_name)
               VALUES (?, ?, ?, ?, ?, ?, ?)""",
            (queue_id, conversation_id, processing_type, priority, user_id, model_id, character_name)
        )
        return queue_id
    
    async def get_processing_queue(self, status: str = "pending", limit: int = 10) -> List[Dict]:
        """Get conversations in processing queue"""
        rows = await self.execute_query(
            """SELECT * FROM conversation_processing_queue 
               WHERE status = ? ORDER BY priority DESC, created_at ASC LIMIT ?""",
            (status, limit)
        )
        return [dict(row) for row in rows]
    
    async def update_processing_status(self, queue_id: str, status: str, created_memory_id: str = None,
                                      error_message: str = None, retry_count: int = 0) -> bool:
        """Update processing queue item status"""
        await self.execute_update(
            """UPDATE conversation_processing_queue 
               SET status = ?, created_memory_id = ?, error_message = ?, retry_count = ?, updated_at = ?
               WHERE queue_id = ?""",
            (status, created_memory_id, error_message, retry_count, get_current_timestamp(), queue_id)
        )
        return True
    
    async def add_chat_isolation_to_conversation(self, conversation_id: str, user_id: str = None,
                                                model_id: str = None, character_name: str = None) -> str:
        """Add chat isolation context to a conversation"""
        isolation_id = str(uuid.uuid4())
        
        await self.execute_update(
            """INSERT OR IGNORE INTO conversation_isolation (isolation_id, conversation_id, user_id, model_id, character_name)
               VALUES (?, ?, ?, ?, ?)""",
            (isolation_id, conversation_id, user_id, model_id, character_name)
        )
        return isolation_id
    
    async def get_isolated_conversations(self, user_id: str = None, model_id: str = None,
                                        character_name: str = None, limit: int = 10) -> List[Dict]:
        """Get conversations filtered by isolation context"""
        if not any([user_id, model_id, character_name]):
            raise ValueError("At least one isolation parameter must be provided")
        
        query = """SELECT c.* FROM conversations c 
                   JOIN conversation_isolation ci ON c.conversation_id = ci.conversation_id 
                   WHERE 1=1"""
        params = []
        ...
        if model_id:
            query += " AND ci.model_id = ?"
            params.append(model_id)
        if character_name:
            query += " AND ci.character_name = ?"
            params.append(character_name)
        
        query += " ORDER BY c.start_timestamp DESC LIMIT ?"
        params.append(limit)
        
        rows = await self.execute_query(query, tuple(params))
        return [dict(row) for row in rows]
    
    async def get_conversation_isolation_context(self, conversation_id: str) -> Optional[Dict]:
        """Get isolation context for a conversation"""
        rows = await self.execute_query(
            "SELECT * FROM conversation_isolation WHERE conversation_id = ?",
            (conversation_id,)
        )
...
class AIMemoryDatabase(DatabaseManager):
    ...
    def initialize_tables(self):
        ...
        with self.get_connection() as conn:
            expected_columns = [
                ...
                'memory_bank', 'embedding', 'created_at'
            ]
            ...
            # Memory Banks table - for organizing memories by category
            conn.execute("""
            ...
            # Tag Registry table - for normalized tag management with canonical forms
            conn.execute("""
            ...
            # Memory-Conversation Junction table - link memories to source conversations
            conn.execute("""
            ...
            # Processing Queue for conversations - track which conversations need memory extraction
            conn.execute("""
                CREATE TABLE IF NOT EXISTS conversation_processing_queue (
                    queue_id TEXT PRIMARY KEY,
                    conversation_id TEXT NOT NULL,
            ...
            # Archived Memories table - for historical preservation of old/summarized memories
            conn.execute("""
            ...
            # Chat Isolation Tracking - for tracking memories by user/model/character combinations
            conn.execute("""
                CREATE TABLE IF NOT EXISTS chat_isolation_context (
                    isolation_id TEXT PRIMARY KEY,
                    memory_id TEXT NOT NULL,
                    user_id TEXT,
                    model_id TEXT,
                    ...
                    created_at TEXT DEFAULT CURRENT_TIMESTAMP,
                    FOREIGN KEY (memory_id) REFERENCES curated_memories (memory_id),
                    UNIQUE(memory_id, user_id, model_id, character_name)
                )
            """)
            ...
            # Initialize default memory banks if not present
            default_banks = [
                ('general_bank', 'General', 'Default general memory bank'),
                ('personal_bank', 'Personal', 'Personal and user-specific memories'),
                ('work_bank', 'Work', 'Work-related memories and tasks'),
            ]
            
            for bank_id, name, description in default_banks:
                conn.execute(
                    "INSERT OR IGNORE INTO memory_banks (bank_id, name, description) VALUES (?, ?, ?)",
                    (bank_id, name, description)
                )
    ...
    async def create_memory(self, content: str, memory_type: str = None, 
        ...
        await self.execute_update(
            """INSERT INTO curated_memories 
               (memory_id, timestamp_created, timestamp_updated, source_conversation_id, 
                memory_type, content, importance_level, tags, memory_bank, embedding) 
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
            (memory_id, timestamp, timestamp, source_conversation_id, 
             memory_type, content, importance_level, 
             json.dumps(tags) if tags else None, memory_bank, embedding_blob)
        )
        
        # Register tags if provided
        if tags:
            for tag in tags:
                await self.register_tag(tag)
        
        return memory_id
    
    async def register_tag(self, tag: str) -> str:
        """Register a tag in the tag registry with canonical form"""
        tag_id = str(uuid.uuid4())
        canonical_form = tag.lower().strip()
        
        # Check if tag already exists
        existing = await self.execute_query(
            "SELECT tag_id FROM tag_registry WHERE canonical_form = ?",
            (canonical_form,)
        )
        ...
        await self.execute_update(
            """INSERT INTO tag_registry (tag_id, canonical_form, variations) 
               VALUES (?, ?, ?)""",
            (tag_id, canonical_form, json.dumps([tag]))
        )
        return tag_id
    
    async def get_memory_banks(self) -> List[Dict]:
        """Get all available memory banks"""
        rows = await self.execute_query("SELECT * FROM memory_banks ORDER BY name")
        return [dict(row) for row in rows]
    
    async def get_or_create_memory_bank(self, name: str, description: str = None) -> str:
        """Get existing memory bank or create new one"""
        bank_id = str(uuid.uuid4())
        
        # Check if bank exists
        existing = await self.execute_query(
            "SELECT bank_id FROM memory_banks WHERE name = ?",
            (name,)
        )
        
        if existing:
            return existing[0]["bank_id"]
        
        await self.execute_update(
            """INSERT INTO memory_banks (bank_id, name, description) VALUES (?, ?, ?)""",
            (bank_id, name, description)
        )
        return bank_id
    
    async def link_memory_to_conversation(self, memory_id: str, conversation_id: str,
                                         message_ids: List[str] = None, link_type: str = "derived",
                                         stub_user_id: str = None, stub_model_id: str = None) -> str:
        """Link a memory to a source conversation
        ...
        if stub_user_id:
            try:
                ...
                if not existing:
                    stub_session_id = str(uuid.uuid4())
                    timestamp = get_current_timestamp()
        ...
        await self.execute_update(
            """INSERT OR IGNORE INTO memory_conversation_links 
               (link_id, memory_id, conversation_id, message_ids, link_type) 
               VALUES (?, ?, ?, ?, ?)""",
            (link_id, memory_id, conversation_id, json.dumps(message_ids) if message_ids else None, link_type)
        )
        return link_id
    
    async def get_memory_conversation_links(self, memory_id: str = None, conversation_id: str = None) -> List[Dict]:
        """Get all links between memories and conversations"""
        if memory_id:
            rows = await self.execute_query(
                "SELECT * FROM memory_conversation_links WHERE memory_id = ?",
                (memory_id,)
            )
        elif conversation_id:
            rows = await self.execute_query(
                "SELECT * FROM memory_conversation_links WHERE conversation_id = ?",
                (conversation_id,)
            )
        else:
            rows = await self.execute_query("SELECT * FROM memory_conversation_links")
        
        return [dict(row) for row in rows]
    
    async def add_to_processing_queue(self, conversation_id: str, session_id: str = None,
                                     processing_type: str = "memory_extraction", priority: int = 5) -> str:
        """Add conversation to processing queue for memory extraction"""
        queue_id = str(uuid.uuid4())
        
        await self.execute_update(
            """INSERT INTO conversation_processing_queue 
               (queue_id, conversation_id, session_id, processing_type, priority) 
               VALUES (?, ?, ?, ?, ?)""",
            (queue_id, conversation_id, session_id, processing_type, priority)
        )
        return queue_id
    ...
    async def update_queue_status(self, queue_id: str, status: str, created_memory_id: str = None, 
                                 error_message: str = None) -> bool:
        """Update status of a queue item"""
        await self.execute_update(
            """UPDATE conversation_processing_queue 
               SET status = ?, created_memory_id = ?, error_message = ?, updated_at = ?
               WHERE queue_id = ?""",
            (status, created_memory_id, error_message, get_current_timestamp(), queue_id)
        )
        return True
    
    async def archive_memory(self, memory_id: str, archive_reason: str = "manual_archive") -> str:
        """Move a memory to archive and track the reason"""
        archive_id = str(uuid.uuid4())
        
        # Get the memory to archive
        memory = await self.execute_query(
            "SELECT * FROM curated_memories WHERE memory_id = ?",
            (memory_id,)
        )
        ...
        await self.execute_update(
            """INSERT INTO archived_memories 
               (archive_id, memory_id, memory_bank, memory_type, content, importance_level, tags, archive_reason, original_created_at)
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)""",
            (archive_id, memory_id, mem.get('memory_bank'), mem.get('memory_type'), mem['content'],
             mem.get('importance_level'), mem.get('tags'), archive_reason, mem.get('timestamp_created'))
        )
        
        return archive_id
    
    async def add_chat_isolation_context(self, memory_id: str, user_id: str = None, 
                                        model_id: str = None, character_name: str = None) -> str:
        """Add chat isolation context to a memory"""
        isolation_id = str(uuid.uuid4())
        
        await self.execute_update(
            """INSERT OR IGNORE INTO chat_isolation_context 
               (isolation_id, memory_id, user_id, model_id, character_name)
               VALUES (?, ?, ?, ?, ?)""",
            (isolation_id, memory_id, user_id, model_id, character_name)
        )
        return isolation_id
    
    async def get_isolated_memories(self, user_id: str = None, model_id: str = None, 
                                   character_name: str = None, limit: int = 10) -> List[Dict]:
        """Get memories filtered by isolation context"""
        if not any([user_id, model_id, character_name]):
            raise ValueError("At least one isolation parameter must be provided")
        
        query = "SELECT m.* FROM curated_memories m JOIN chat_isolation_context c ON m.memory_id = c.memory_id WHERE 1=1"
        params = []
    ...
    async def delete_memory(self, memory_id: str) -> Dict:
        ...
        try:
            ...
            return {
                "status": "success" if result > 0 else "not_found",
                "memory_id": memory_id,
                "deleted": result > 0
            }
        except Exception as e:
    ...
    async def list_available_memory_banks(self) -> Dict:
        """List all available memory banks with memory counts"""
        try:
            banks_query = """
                SELECT COALESCE(memory_bank, 'General') as bank_name, COUNT(*) as memory_count
                FROM curated_memories
                GROUP BY memory_bank
                ORDER BY memory_count DESC
            """
            
            results = await self.execute_query(banks_query, ())
            
            # Ensure we have at least the default General bank
            bank_dict = {row["bank_name"]: row["memory_count"] for row in results}
            if "General" not in bank_dict:
                bank_dict["General"] = 0
            
            return {
                "status": "success",
                "memory_banks": bank_dict,
                "total_banks": len(bank_dict),
                "total_memories": sum(bank_dict.values())
            }
        except Exception as e:
    ...
    async def list_available_tags(self) -> Dict:
        """List all available tags with usage counts"""
        try:
            tags_query = """
                SELECT 
                    canonical_form,
                    COUNT(cm.memory_id) as usage_count
                FROM tag_registry tr
                LEFT JOIN curated_memories cm ON cm.tags LIKE '%' || tr.canonical_form || '%'
                GROUP BY tr.canonical_form
                ORDER BY usage_count DESC
            """
            
            results = await self.execute_query(tags_query, ())
            
            # Build tag dictionary
            tag_dict = {row["canonical_form"]: row["usage_count"] for row in results}
            
            return {
                "status": "success",
                "tags": tag_dict,
                "total_tags": len(tag_dict),
                "total_usages": sum(tag_dict.values())
            }
        except Exception as e:
...
class ConversationFileMonitor:
    ...
    async def _extract_conversations(self, file_path: str) -> List[Dict]:
        ...
        try:
            ...
            # Special handling for JSONL format
            if file_path.lower().endswith('.jsonl'):
                conversations.extend(await self._parse_jsonl_format(file_path))
                return conversations
            ...
            format_handlers = [
                ...
                (lambda fn, _: fn.endswith('.md'), self._parse_markdown_format),
                (lambda fn, _: fn.endswith(('.txt', '.log')), self._parse_text_format),
            ]
            ...
            for predicate, handler in format_handlers:
                if predicate(file_path, content):
                    ...
                    elif handler == self._parse_markdown_format:
                        conversations.extend(handler(content))
                    else:
                        conversations.extend(handler(content))
                    handled = True
            ...
            # Enhanced deduplication: by id, timestamp, and content hash
            # Also tracks semantic duplicates if needed
            deduped = await self._deduplicate_conversations(conversations)
            return deduped
        except Exception as e:
            logger.error(f"Error extracting conversations from {file_path}: {e}")
            return []
    
    async def _deduplicate_conversations(self, conversations: List[Dict]) -> List[Dict]:
        """Deduplicate conversations using multiple strategies:
        - Exact match: id, timestamp, content hash
        - Optional semantic match: embedding-based similarity (if enabled)
        """
        ...
        for conv in conversations:
            ...
            cid = conv.get('id') or conv.get('message_id')
            ...
            if dedup_key not in seen:
                ...
                logger.debug(f"Skipped duplicate message: {dedup_key}")
    ...
    def _parse_claude_export_format(self, data: Dict) -> List[Dict]:
        """Parse Anthropic Claude conversation export format"""
        conversations = []
        
        try:
            # Claude exports as a dictionary with conversation turns
            if 'conversation' in data:
                for turn in data['conversation']:
                    if isinstance(turn, dict) and 'content' in turn:
                        conversations.append({
                            ...
                            'timestamp': self.parse_timestamp(turn.get('created_at') or turn.get('timestamp')),
                            'metadata': {'source_type': 'claude_export'}
                        })
                ...
                for msg in data['messages']:
                    if isinstance(msg, dict):
                        ...
                        if isinstance(content, list):
                            # Claude may use content blocks
                            content = ' '.join(str(block.get('text', block)) for block in content if isinstance(block, dict))
                        else:
                            content = str(content)
                        ...
                        if content:
                            conversations.append({
                                'role': role,
                                'content': content,
                                'timestamp': self.parse_timestamp(msg.get('created_at') or msg.get('timestamp')),
                                'metadata': {'source_type': 'claude_export'}
                            })
        except Exception as e:
            logger.warning(f"Error parsing Claude export format: {e}")
        
        return conversations
    
    def _parse_markdown_format(self, content: str) -> List[Dict]:
        """Parse markdown conversation format with alternating user/assistant messages"""
        conversations = []
        ...
        try:
            ...
            for line in lines:
                ...
                if line.startswith('## ') and (':' in line):
                    ...
                    header = line[3:].lower()  # Remove '## '
                    if 'user' in header:
                        current_role = 'user'
                    elif 'assistant' in header or 'ai' in header or 'response' in header:
                        current_role = 'assistant'
                    else:
                        # Try to extract anything before the colon
            ...
            logger.warning(f"Error parsing markdown format: {e}")
        
        return conversations
    
    async def _parse_jsonl_format(self, file_path: str) -> List[Dict]:
        """Parse JSONL (JSON Lines) conversation format - one JSON object per line"""
        conversations = []
                            ...
                            if isinstance(obj, dict):
                                ...
                                if 'role' in obj and 'content' in obj:
                                    conversations.append({
                                        ...
                                        'metadata': {'source_type': 'jsonl', 'line_number': line_num}
                                    })
                                elif 'message' in obj:
...
class PersistentAIMemorySystem:
    ...
    async def delete_memory(self, memory_id: str) -> Dict:
        """Delete a memory by ID"""
        return await self.ai_memory_db.delete_memory(memory_id)
    
    async def list_available_memory_banks(self) -> Dict:
        """List all available memory banks with memory counts"""
        return await self.ai_memory_db.list_available_memory_banks()
    
    async def list_available_tags(self) -> Dict:
        """List all available tags with usage counts"""
        return await self.ai_memory_db.list_available_tags()
    ...
    async def trigger_database_maintenance(self, force: bool = True) -> Dict:
        """Wrapper for manually triggering database maintenance via MCP.
        
        This allows MCP clients to trigger maintenance operations on demand,
        outside of the regular 6-hour scheduled maintenance.
        
        Args:
            force: Whether to force maintenance to run immediately (default True)
            
        Returns:
            Dict containing maintenance status and results
        """
        try:
            logger.info(f"Database maintenance triggered via MCP (force={force})")
            results = await self.run_database_maintenance(force=force)
            return {
                "status": "success",
            ...
            logger.error(f"Error triggering database maintenance: {e}")
            return {
                "status": "error",
                "message": str(e),
                "timestamp": get_current_timestamp()
    ...
    async def export_all_tool_calls(self, output_filename: str = None) -> Dict:
        """Export all tool calls from databases for LORA training dataset generation.
        
        Exports complete tool call history with parameters, results, and metadata
        for use in training datasets or analysis. This includes archived data.
        
        Args:
            output_filename: Optional custom filename. If not provided, uses timestamp-based name
            
        Returns:
            Dict containing export status, file path, and statistics
        """
        try:
            import json
            ...
            logger.info("Exporting all tool calls for LORA training dataset")
            
            # Get all tool calls from MCP database
            all_calls = await self.mcp_db.execute_query(
                """SELECT * FROM tool_calls 
                   ORDER BY timestamp DESC"""
            )
            
            # Get summary statistics
            summary_stats = await self.mcp_db.get_tool_usage_summary(days=365)
            ...
            export_data = {
                ...
                "tool_calls": [dict(call) for call in all_calls]
            }
            
            # Determine output filename
            if not output_filename:
                timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
                output_filename = f"tool_calls_export_{timestamp}.json"
            ...
            logger.info(f"Tool calls exported to {output_path}")
            
            return {
                "status": "success",
                "export_file": str(output_path),
                "tool_calls_exported": len(all_calls),
                "timestamp": get_current_timestamp(),
    ...
    async def brave_web_search(self, query: str, count: int = 10, country: str = "US", language: str = "en") -> Dict:
        ...
        NOTE: This requires the Brave Search API key to be configured.
        See README.md for setup instructions.
        
        Args:
            query: Search query string
            count: Number of results (1-20, default 10)
            country: Country code (e.g., 'US', 'CA', default 'US')
        ...
        try:
            ...
            if not api_key:
                ...
                config_file = Path("keys") / "brave_api_key.txt"
            ...
            if not api_key:
                return {
                    ...
                    "error": "Brave API key not configured. Please see README.md for setup instructions.",
                    "documentation": "To use Brave Web Search, you need to: 1) Get an API key from https://api.search.brave.com 2) Create keys/brave_api_key.txt with content: Brave_API_Key=\"your-key-here\" OR set BRAVE_API_KEY environment variable"
                }

            # Limit count to reasonable bounds
            count = min(max(count, 1), 20)

            # NOTE: Actual API call would require aiohttp and proper implementation
            # This is a stub that documents the requirement
            logger.warning(f"Brave web search called but API integration not fully configured: query={query}")
            
            return {
                "status": "error",
                "error": "Brave Search API integration requires additional setup. See README.md",
                "query": query,
                "note": "This is a stub implementation. To enable Brave search, implement the aiohttp-based API client."
            }
            ...
            return {
                "status": "error",
                "message": str(e),
                "documentation": "See README.md for Brave Search setup instructions"
            }
    ...
    async def brave_local_search(self, query: str, location: str = None, count: int = 10, radius: int = 5000) -> Dict:
        ...
        Args:
            query: Search query (e.g., 'pizza', 'coffee shops')
        ...
        try:
            ...
            if not api_key:
                return {
                    "status": "not_configured",
                    "error": "Brave API key not configured. Please see README.md for setup instructions.",
                    "documentation": "To use Brave Local Search, you need to: 1) Get an API key from https://api.search.brave.com 2) Create keys/brave_api_key.txt with content: Brave_API_Key=\"your-key-here\" OR set BRAVE_API_KEY environment variable"
                }
            ...
            # NOTE: Actual API call would require aiohttp and proper implementation
            # This is a stub that documents the requirement
            logger.warning(f"Brave local search called but API integration not fully configured: query={query}, location={location}")
            ...
            return {
                ...
                "query": query,
                "location": location,
                "note": "This is a stub implementation. To enable Brave local search, implement the aiohttp-based API client."
            }
            
        except Exception as e:
            logger.error(f"Error in brave_local_search: {e}")
            return {
                "status": "error",
                "message": str(e),
                "documentation": "See README.md for Brave Search setup instructions"
    ...
    async def get_tool_information(self, mode: str = "usage", days: int = 7, client_id: str = None, tool_name: str = None) -> Dict:
        """Get tool usage statistics OR tool documentation.
        
        Args:
            mode: "usage" (default) for statistics, "documentation" for tool descriptions
            days: For usage mode - analyze past N days (default 7)
            client_id: For usage mode - specific client to analyze (optional)
            tool_name: For documentation mode - specific tool to document (optional)
            
        Returns:
            Dict containing either usage statistics or tool documentation
        """
        try:
            if mode == "documentation":
                # Return tool documentation
                return await self._get_tool_documentation(tool_name)
            else:
                # Return usage statistics
                usage_summary = await self.get_tool_usage_summary(days=days)
    ...
    async def _get_tool_documentation(self, tool_name: str = None) -> Dict:
        """Generate documentation for available tools.
        
        Args:
            tool_name: Optional specific tool to document. If not provided, returns all tools.
            
        Returns:
            Dict containing tool documentation
        """
        # Complete list of tools organized by category
        tools_by_category = {
            "Memory Operations": [
                {"name": "search_memories", "description": "Search memories using semantic similarity with importance and type filtering"},
                {"name": "create_memory", "description": "Create and store a curated memory with optional embedding generation"},
                {"name": "update_memory", "description": "Update an existing memory's content, importance, or tags"},
                {"name": "delete_memory", "description": "Permanently delete a memory from the system"}
            ],
            "Memory Management": [
                {"name": "list_available_memory_banks", "description": "Get list of memory categories with counts"},
                {"name": "list_available_tags", "description": "Get list of all tags with usage statistics"}
            ],
            "Schedule Management": [
                {"name": "create_appointment", "description": "Create appointments with optional recurrence"},
                {"name": "create_reminder", "description": "Create reminders with priority levels"},
                {"name": "get_upcoming_appointments", "description": "Get appointments scheduled within N days"},
                {"name": "get_appointments", "description": "Get recent appointments optionally filtered by date"},
                {"name": "get_reminders", "description": "Get active reminders"},
                {"name": "get_active_reminders", "description": "Get active (not completed) reminders"},
                {"name": "get_completed_reminders", "description": "Get recently completed reminders"},
                {"name": "complete_reminder", "description": "Mark a reminder as completed"},
                {"name": "reschedule_reminder", "description": "Update the due date of a reminder"},
                {"name": "delete_reminder", "description": "Permanently delete a reminder"},
                {"name": "cancel_appointment", "description": "Cancel a scheduled appointment"},
                {"name": "complete_appointment", "description": "Mark an appointment as completed"}
            ],
            "System Operations": [
                {"name": "get_system_health", "description": "Get comprehensive system health and database statistics"},
                {"name": "get_tool_usage_summary", "description": "Get AI tool usage summary for analysis"},
                {"name": "reflect_on_tool_usage", "description": "AI self-reflection on tool usage patterns"},
                {"name": "store_ai_reflection", "description": "Store AI self-reflection records"},
                {"name": "write_ai_insights", "description": "Alias for store_ai_reflection"},
                {"name": "get_ai_insights", "description": "Get recent AI insights and reflections"},
                {"name": "get_current_time", "description": "Get current server time in UTC and local timezone"},
                {"name": "get_weather_open_meteo", "description": "Get weather forecast (no API key required)"}
            ],
            "Conversation & Context": [
                {"name": "store_conversation", "description": "Store conversation messages with automatic embedding"},
                {"name": "get_recent_context", "description": "Get recent conversation context"}
            ],
            "Project Development": [
                {"name": "save_development_session", "description": "Save VS Code development session context"},
                {"name": "store_project_insight", "description": "Store development insights and decisions"},
                {"name": "search_project_history", "description": "Search development history and insights"},
                {"name": "link_code_context", "description": "Link conversation to specific code context"},
                {"name": "get_project_continuity", "description": "Get context for continuing development work"}
            ],
            "External Search": [
                {"name": "brave_web_search", "description": "General web search using Brave Search API (requires API key - see README)"},
                {"name": "brave_local_search", "description": "Search local businesses using Brave Search API (requires API key - see README)"},
                {"name": "get_tool_information", "description": "Get tool documentation and usage information"}
            ],
            "Character/Roleplay": [
                {"name": "get_character_context", "description": "Get relevant context about characters from memory (SillyTavern)"},
                {"name": "store_roleplay_memory", "description": "Store roleplay moments and character developments"},
                {"name": "search_roleplay_history", "description": "Search past roleplay interactions"}
            ],
            "Maintenance": [
                {"name": "trigger_database_maintenance", "description": "Manually trigger database optimization and maintenance"},
                {"name": "export_all_tool_calls", "description": "Export tool call history for training datasets"}
            ]
        }
        ...
        if tool_name:
                ...
                for tool in tools:
                    if tool["name"] == tool_name:
                        return {
                            ...
                            "category": category,
                            "documentation": tool,
                            "note": "For implementation details, see source code in ai_memory_core.py"
                        }
            
            return {
                "status": "not_found",
                "error": f"Tool '{tool_name}' not found",
                "available_tools": list(tools_by_category.keys())
            }
        else:
            # Return all tools
            all_tools = []
            ...
            return {
                ...
                "tools": all_tools,
                "by_category": {k: len(v) for k, v in tools_by_category.items()}
            }
    ...
    async def reflect_on_tool_usage(self, days: int = 7, client_id: str = None) -> Dict:
        ...
        Analyzes tool usage statistics from the past N days and generates
        insights about usage patterns, effectiveness, and recommendations.
        
        Args:
            days: Number of days to analyze (default 7)
            client_id: Optional specific client to analyze
            
        Returns:
            Dict containing reflection results with insights and recommendations
        """
        try:
            ...
            most_used = usage_summary.get("most_used_tools", [])
            daily_stats = usage_summary.get("daily_stats", [])
            recent_calls = usage_summary.get("recent_calls", [])
            
            # Calculate statistics
            total_calls = sum(tool.get("total_calls", 0) for tool in most_used)
            success_calls = sum(call.get("count", 0) for call in recent_calls if call.get("status") == "success")
            error_calls = sum(call.get("count", 0) for call in recent_calls if call.get("status") == "error")
            success_rate = (success_calls / (success_calls + error_calls) * 100) if (success_calls + error_calls) > 0 else 0
            ...
            if most_used:
                top_tool = most_used[0]["tool_name"]
                top_count = most_used[0]["total_calls"]
                insights.append(f"Most used tool: {top_tool} ({top_count} calls)")
            ...
            if error_calls > 0:
                error_rate = (error_calls / (success_calls + error_calls) * 100)
                insights.append(f"Error rate: {error_rate:.1f}% ({error_calls} errors)")
                confidence_level = 0.7  # Lower confidence if there are errors
                
                # Find problematic tools
                error_tools = [call["tool_name"] for call in recent_calls if call.get("status") == "error"]
                if error_tools:
                    unique_error_tools = set(error_tools)
                    recommendations.append(f"Review error handling for: {', '.join(unique_error_tools)}")
            
            # Insight 4: Usage trend
            if daily_stats and len(daily_stats) > 1:
                recent_usage = daily_stats[0].get("call_count", 0) if daily_stats else 0
                older_usage = daily_stats[-1].get("call_count", 0) if daily_stats else 0
                if older_usage > 0:
                    trend = ((recent_usage - older_usage) / older_usage * 100)
                    if trend > 20:
                        insights.append(f"Usage trending up: +{trend:.1f}%")
                    elif trend < -20:
                        insights.append(f"Usage trending down: {trend:.1f}%")
            
            # Recommendations
            if total_calls < 10:
                recommendations.append("Consider increasing usage of memory and scheduling tools")
            
            if len(most_used) > 3:
                tools_to_explore = [tool["tool_name"] for tool in most_used[3:6]]
                recommendations.append(f"Explore underutilized tools: {', '.join(tools_to_explore)}")
            
            if success_rate < 90:
                recommendations.append("Focus on reducing error rates through better error handling")
            
            # Create reflection content
            content = f"Tool Usage Analysis ({days} days):\n"
            content += f"- Total calls: {total_calls}\n"
            content += f"- Success rate: {success_rate:.1f}%\n"
            content += f"- Most used: {most_used[0]['tool_name'] if most_used else 'N/A'}\n"
            content += f"- Period: Last {days} days"
            
            # Store the reflection
            reflection_result = await self.store_ai_reflection(
                content=content,
            ...
            return {
                ...
                "analysis": {
                    ...
                    "error_rate": (error_calls / (success_calls + error_calls) * 100) if (success_calls + error_calls) > 0 else 0,
