...
# Tag Manager for dynamic tag registry
try:
...
try:
    ...
    def _get_conversation_db() -> ConversationDatabase:
        return ConversationDatabase(str(get_settings().conversations_db_path))
...
What details are relevant to remember about this image? Describe only the key visual information that would be important to recall later in this context. Keep it brief but specific (2-3 sentences max)."""
    ...
    class Valves(BaseModel):
        ...
        core_identity_batch_size: int = Field(
            default=50,
            description="Memories per batch during identity generation. Larger values process more per LLM call but use more context.",
        )
    ...
    def __init__(self):
        ...
        try:
            if TAG_MANAGER_AVAILABLE:
                ...
                self._tag_registry_filepath = os.path.join(get_memory_data_dir(), "tag_registry.json")
                if os.path.exists(self._tag_registry_filepath):
                    ...
                    logger.info("Tag registry file not found, will build from memories on startup")
            else:
        ...
        # Schedule tag registry build from existing memories
        if TAG_MANAGER_AVAILABLE and getattr(self, 'tag_manager', None):
        ...
        # Schedule core identity generation task (now handled by TaskCoordinator)
        if self.valves.enable_core_identity_task:
            logger.debug("Core identity generation managed by TaskCoordinator (interval:12h,idle)")
        ...
        if self.valves.enable_error_logging_task:
            coordinator_registrations.append(("error_logging", self._error_logging_work, "interval:30m", "db_light"))
        if self.valves.enable_retry_queue_task:
        ...
        if self.valves.enable_model_discovery_task:
            coordinator_registrations.append(("model_discovery", self._model_discovery_work, "interval:2h", "db_light"))

        coordinator_registrations.extend([
            ("summarization", self._summarization_work, "interval:2h,idle", "llm"),
            ("core_identity", self._core_identity_work, "daily@00:00,idle", "llm"),
            ("memory_linking", self._memory_linking_work, "daily@02:00,idle", "db_heavy", "conversations"),
            ("registry_sync", self._registry_sync_work, "interval:2h,idle", "db_heavy", "memories"),
            ("nightly_sync", self._nightly_sync_work, "daily@02:30,idle", "db_heavy", "memories"),
            ("memory_promotion", self._memory_promotion_work, "interval:24h,idle", "db_heavy", "both"),
            ("maintenance_mcp", self._maintenance_work, "interval:6h,anchor=03:00", "db_heavy", "conversations"),
            ("maintenance_24h", self._maintenance_work, "daily@05:00", "db_heavy", "conversations"),
            ("linking_validation", self._linking_validation_work, "interval:6h,anchor=04:30", "db_heavy", "conversations"),
            ("openwebui_import", self._openwebui_import_work, "interval:3h", "db_light"),
        ])
        ...
        for reg in coordinator_registrations:
            ...
            logger.debug(f"Coordinator registered '{name}' ({schedule})")

        self._background_tasks.add(asyncio.create_task(self._coordinator.start()))

        # Valve-gated task registration guard — ensures persisted settings are saved once
        ...
        # Schedule date update task if enabled (now handled by TaskCoordinator)
        if self.valves.enable_date_update_task:
            logger.debug("Date update managed by TaskCoordinator (interval:1h)")
        else:
            logger.debug("Date update task disabled by valve")

        # Schedule model discovery task if enabled (now handled by TaskCoordinator)
        if self.valves.enable_model_discovery_task:
            logger.debug("Model discovery managed by TaskCoordinator (interval:2h)")
        else:
            logger.debug("Model discovery task disabled by valve")
...
Produce ONLY the corrected JSON output following the format specified in the system prompt."""
    ...
    async def _memory_promotion_work(self):
        ...
        try:
            if not AI_MEMORY_SYSTEM_AVAILABLE:
                logger.warning("AI Memory System not available, skipping promotion")
                return
    ...
    async def _memory_linking_work(self):
        ...
        """
        cursor_path = os.path.join(get_memory_data_dir(), "memory_linking_cursor.json")
                ...
                for memory in memories:
                    try:
                        ...
                        if best_match:
                            ...
                            logger.debug(f"No matching conversation found for memory {memory_id} (user={user_id}, model={model_id})")
    ...
    async def _core_identity_work(self):
        ...
        Queries OpenWebUI DB directly for all users and all models with the
        short-term memory filter enabled. Iterates every (user, model) pair —
        no hardcoded names, no fallback strings, no reliance on in-memory state.
        """
        try:
            from core_identity import CoreIdentityManager
            from core_identity import OPENWEBUI_DB_PATH
            memory_data_dir = os.getenv("AI_MEMORY_DATA_DIR", "./memory_data")
            manager = CoreIdentityManager(memory_data_dir=memory_data_dir, request=getattr(self, "request", None))
            manager.initialize()
            ...
            model_rows = conn.execute(
                "SELECT id, name FROM model WHERE meta LIKE ?",
                ('%"filterIds"%"friday_short_term_memory"%',)
            ).fetchall()
            conn.close()

            enabled_models = {r["id"].strip().lower(): r["name"] for r in model_rows if r["id"]}

            if not user_rows:
                logger.info("Core identity: no users found, skipping")
                return

            if not enabled_models:
                logger.info("Core identity: no models with short-term filter enabled, skipping")
                return
            ...
            for user_row in user_rows:
                ...
                for model_id, model_display_name in enabled_models.items():
                    if self._coordinator and not self._coordinator.is_idle():
                        logger.info("Core identity: system not idle, stopping iteration")
                        return

                    logger.info(f"Core identity generation: user={user_name}({user_id}), model={model_id}")
                    result = await manager.run_generation(
                        ...
                        batch_size=self.valves.core_identity_batch_size,
                        user_name=user_name,
                    ...
                    if result["status"] == "completed":
                        log_entry = (
                            ...
                            f"CORE_IDENTITY: Generation completed - user={user_name}({user_id}), "
                            f"model={model_id}, version={result.get('version', 0)}, "
                        ...
                        logger.warning(f"Core identity generation failed for user={user_name} model={model_id}: {result.get('error', 'unknown')}")
        except Exception as e:
    ...
    async def _perform_registry_sync(self) -> None:
        try:
            ...
            tag_registry_path = os.path.join(memory_data_dir, "tag_registry.json")
            bank_registry_path = os.path.join(memory_data_dir, "SystemMarkers", "bank_registry.json")
            ...
            if not getattr(self, 'tag_manager', None):
                logger.debug("Tag manager not available, skipping registry sync")
                return

            ai_memories_db_path = os.path.join(memory_data_dir, "ai_memories.db")
            ...
            if missing_tags:
                ...
                success = self.tag_manager.save_registry(self.tag_manager.registry, tag_registry_path)
                if success:
                    logger.info(f"✓ Updated tag registry with {len(missing_tags)} new tags")
                else:
                    logger.warning(f"Failed to save tag registry to {tag_registry_path}")
            else:
                logger.debug("No missing tags found, tag registry is up to date")
            ...
            if missing_banks:
                ...
                for bank in missing_banks:
                    if bank not in existing_bank_names:
                        bank_data.setdefault("banks", []).append({
                            "name": bank.capitalize(),
                            "description": f"Auto-discovered bank: {bank}",
                            "discovered": True
                        })
                ...
                if hasattr(self, '_discovered_memory_banks'):
                    self._discovered_memory_banks.update(missing_banks)
            else:
                logger.debug("No missing banks found, bank registry is up to date")
        except Exception as e:
...
async def inlet(
        ...
        try:
            ...
            if loaded_config_valves is not None:
                ...
                # Persist current valve settings so next boot loads correct values.
                # All tasks are registered unconditionally in PAM, so no re-registration needed.
    ...
    async def _build_tag_registry_from_memories(self) -> None:
        try:
            if not getattr(self, 'tag_manager', None):
                ...
                return
            if not hasattr(self, 'ai_memory_db') or self.ai_memory_db is None:
                logger.debug("Memory database not available, skipping tag registry build")
                return
            query = "SELECT memory_id, content, tags FROM curated_memories LIMIT 10000"
            try:
                rows = await self.ai_memory_db.execute_query(query, ())
                ...
                if memories:
                    ...
                    if self.tag_manager.registry:
                        ...
                        if success:
                            ...
                            logger.warning(f"Failed to save tag registry to {self._tag_registry_filepath}")
                    else:
    ...
    async def _inject_core_identity_into_context(
        ...
        try:
            ...
            if identity_text:
                ...
                logger.info(
                    f"CORE IDENTITY NOT FOUND: user={user_id}, model={model_id} — injection skipped"
                )
        except Exception as e:
                                ...
                                try:
                                    ...
                                    if memory_content:
                                        ...
                                        try:
                                            ...
                                            mem_db_path = getattr(memory_system, 'memory_db_path', None) or _FRIDAY_MEMORIES_DB
                                            link_db = AIMemoryDatabase(mem_db_path)
...
"""
            ...
            # Inject tag registry into the prompt context
            tags_context = ""
            if getattr(self, 'tag_manager', None) and self.tag_manager and self.tag_manager.registry:
                available_tags = sorted(self.tag_manager.registry.keys())
                tags_str = ", ".join(available_tags[:20])
    ...
    async def _execute_memory_operation(
        ...
        if operation.operation == "NEW":
            try:
                ...
                # Update tag registry with tags from this new memory
                if getattr(self, 'tag_manager', None) and tags_for_save:
                    try:
                        memory_dict = {
                            "content": formatted_content,
                            "tags": tags_for_save
                        ...
                        if new_registry:
                            ...
                            if success:
                                ...
                                logger.warning(f"Failed to save updated tag registry to {self._tag_registry_filepath}")
                    except Exception as e:
