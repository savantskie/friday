...
#### Brave Search API (Web & Local Search)

The system includes optional web search and local business search capabilities using the Brave Search API. These tools are available but require API key configuration.
...
2. **Configure the API Key**:
   ...
   **Option B: Using Configuration File**
   - Copy the template: `cp keys/brave_api_key.txt.example keys/brave_api_key.txt`
   - Edit `keys/brave_api_key.txt` and replace `YOUR_BRAVE_API_KEY_HERE` with your actual key
   - Format must be: `Brave_API_Key="your-key-here"`

3. **Verify Configuration**:
   - Once configured, the `brave_web_search` and `brave_local_search` tools will become fully functional
   - If the API key is not configured, these tools will return a helpful error message directing to this documentation

**Available Search Tools:**
- `brave_web_search(query, count=10, country="US", language="en")` - General web search
- `brave_local_search(query, location=None, count=10, radius=5000)` - Local business search

**Note**: These tools are optional. The system functions completely without them; they're only needed if you want web or local search capabilities.
