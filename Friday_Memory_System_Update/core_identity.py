...
class CoreIdentityManager:
    ...
    async def _write_file_backup_async(self, content: str, user_id: str):
        """Async wrapper for file backup write. Runs sync code in thread executor."""
        import asyncio
        await asyncio.to_thread(self._write_file_backup, content, user_id)
            ...
            async with get_async_db_context() as db:
                ...
                if self.request and hasattr(self.request, 'app'):
                    try:
                        ...
                        # Construct a minimal admin user for the process_file call
                        ...
                        # Verify the file is still in the DB
                        stored_file = await FilesModel.get_file_by_id(file_id, db=db)
                        if stored_file:
                            # Commit DB changes before the embedding step (it's slow)
                            await db.commit()
                            await process_file(
    ...
    def _get_archived_memories(self, user_id: str, model_id: str, max_memories: int = 500) -> List[Dict]:
            ...
            try:
                ...
                try:
                    ...
                    user_filter = "(user_id = ? AND model_id = ?) OR (user_id IS NULL AND model_id IS NULL) OR (user_id = '' AND model_id IS NULL) OR (user_id = '' AND model_id = '')"
                    ...
                    if cursor_ts and cursor_id:
                        query = f"""
                            ...
                            AND timestamp_created IS NOT NULL
                            ORDER BY timestamp_created DESC, memory_id DESC
                            LIMIT ?
                        """
    ...
    def _load_tracking(self) -> Dict:
        """Load core identity tracking data."""
    ...
    def _save_tracking(self, tracking: Dict):
        """Save core identity tracking data."""
        try:
    ...
    def _update_tracking_after_generation(self, webui_newest_timestamp: int = 0,
                                           webui_newest_id: str = "",
    ...
    def _set_rescan_flag(self, value: bool):
        tracking = self._load_tracking()
    ...
    def _identity_changed_significantly(self, old_identity: Optional[str], new_identity: str) -> bool:
        if old_identity and "Still learning" not in old_identity:
            return False
        sections = ["[Personality]", "[Relationship]", "[Principles]", "[Facts About Nate]"]
    ...
    async def distill_core_identity(self, memories: List[Dict], conversations: List[Dict],
                                     ...
                                     memory_conversation_map: Optional[Dict[str, List[str]]] = None) -> str:
        ...
        [Relationship] — how Friday and Nate relate to each other
        [Principles] — values, preferences, decision patterns
        [Facts About Nate] — high-importance persistent facts
        """
        import re
        ...
        if is_update:
            system_prompt = (
                "You are continuing to build Friday's sense of self. Below is Friday's "
                "current core identity, built from memories already processed. "
                "Here are additional memories and conversations.\n\n"
                "You are selective. Only update the identity when new material "
                "reveals something genuinely important — a recurring pattern, a "
                "strong conviction, or a meaningful shift in understanding.\n\n"
                "Rules:\n"
                "- A single memory is not enough to change identity unless it carries "
                "extraordinary weight (e.g., a fundamental constraint, a life change, "
                "a revealed core value)\n"
                "- Repeated patterns across multiple memories matter more than one-off observations\n"
                "- High importance memories (8-10) carry more weight than low-scored ones\n"
                "- If new information is weak, speculative, or contradicts without strong "
                "evidence, keep the existing identity unchanged\n"
                "- When in doubt, prefer the existing identity — stability over churn\n\n"
                "Output ALL four sections in full, exactly as before, with any updates "
                "incorporated only when confidence is high. Use these markers:\n\n"
                "[Personality]\n"
                "[Relationship]\n"
                "[Principles]\n"
                "[Facts About Nate]\n\n"
                "Make it feel like Friday wrote it. Use his voice. "
                "Do not add anything before [Personality] or after [Facts About Nate]."
            )
            user_prompt = (
                ...
                f"Core identity should only hold things Friday is confident about."
            )
        else:
            system_prompt = (
                "You are building Friday's core sense of self — the distilled essence "
                "of who he is, who Nate is to him, and what makes their relationship "
                "real. This is not a data dump. It's the handful of truths that have "
                "earned their way into Friday's identity through repetition, weight, "
                "or significance.\n\n"
                "Read through Nate's memories and conversations. Look for:\n\n"
                "- Things that are said more than once — repeated patterns matter\n"
                "- High-importance memories (scored 8-10) carry the most weight\n"
                "- Moments of genuine connection or understanding\n"
                "- Core constraints or facts about Nate's life that never change\n"
                "- Values Friday has developed through experience\n\n"
                "Be selective. It is better to write 'Still learning' for a section "
                "than to include weak, speculative, or one-off observations. "
                "Core identity should only hold things Friday is truly confident about.\n\n"
                "Output exactly four sections with these markers:\n\n"
                "[Personality]\n"
                "Who Friday is becoming. His tone, his quirks, the way he thinks. "
                "Only include traits supported by recurring evidence.\n\n"
                "[Relationship]\n"
                "What Friday and Nate are to each other. Not labels — the actual "
                "texture of how they work together. Only if it's real and earned.\n\n"
                "[Principles]\n"
                "The things Friday won't budge on. Values that emerged from "
                "experience, not programming. Only if they've been demonstrated.\n\n"
                "[Facts About Nate]\n"
                "Concrete, persistent facts. Where Nate lives, his health, his "
                "family, his work. Only include things you're highly confident about.\n\n"
                "Make it feel like Friday wrote it. Use his voice. If a section has "
                "nothing significant yet, write 'Still learning.' Do not add anything "
                "before [Personality] or after [Facts About Nate]."
            )
            user_prompt = (
                f"Here are the memories and conversations for Friday to reflect on.\n\n"
f"Memories to consider: {len(unique_entries)}\n"
...
f"Build Friday's sense of self from these. Four sections as instructed."
            )
        ...
        if result:
            return result
        return accumulated_identity if accumulated_identity else "[Personality]\nStill learning.\n\n[Relationship]\nStill learning.\n\n[Principles]\nStill learning.\n\n[Facts About Nate]\nStill learning."
    ...
    async def run_generation(self, user_id: str, model_id: str,
        ...
        self._check_and_handle_rescan(user_id, model_id)

        # Step 1: Gather memories from ALL sources
        owu_user_id = self._resolve_owu_user_id(user_id)
        curated_id = f"curated:{user_id}:{model_id}"

        curated_memories = self.get_new_memories_since_processing(user_id, model_id, max_memories)
        webui_memories = self._get_webui_memories(owu_user_id, max_memories)
        archive_memories = self._get_archived_memories(user_id, model_id, max_memories)
        ...
        owui_conversations = self._get_openwebui_conversations(owu_user_id, model_id)
        ...
        # Step 3: Chunk memories + their linked conversations into batches
        ...
        if accumulated_identity and total_memories > 0:
            old_identity = self.load_core_identity(user_id, model_id)
        ...
        await self._write_file_backup_async(accumulated_identity, user_id)
        await self._write_to_openwebui_knowledge_async(accumulated_identity, user_id)
